...
import type { IUserSkill } from '../models/user.model.js';
...
const generateAccessAndAccessTokens = async (userId: Types.ObjectId): Promise<TokenPair> => {
  try {
...
export const register = asyncHandler(async (req, res) => {
  ...
  return res.status(201).cookie("accessToken", accessToken, COOKIE_OPTIONS)
...
export const getProfile = asyncHandler<AuthenticatedRequest>(async (req, res) => {
...
export const updateProfile = asyncHandler<AuthenticatedRequest>(async (req, res) => {
  const { name, phone, skills, isVulnerable } = req.body as {
    name?: string; phone?: string; skills?: IUserSkill[]; isVulnerable?: boolean;
  };
...
export const addGuardian = asyncHandler<AuthenticatedRequest>(async (req, res) => {
  ...
  if (guardian._id.toString() === req.user._id.toString()) {
    throw new ApiError(400, 'You cannot add yourself as guardian');
  }
...
export const removeGuardian = asyncHandler<AuthenticatedRequest>(async (req, res) => {
...
export const getGuardians = asyncHandler<AuthenticatedRequest>(async (req, res) => {
  const user = await User.findById(req.user._id)
    .populate('guardians', 'name email phone avatar')
...
export const getWards = asyncHandler<AuthenticatedRequest>(async (req, res) => {
  // Find users who have me as their guardian
  const wards = await User.find({ guardians: req.user._id })
    .select('name email phone avatar isVulnerable');
