...
export const createSOS = asyncHandler(async (req, res) => {
  ...
  const allowedRadius = [500, 1000, 2000];
  const normalizedRadius = Number(broadcastRadius) || 1000;
  if (!allowedRadius.includes(normalizedRadius)) {
...
export const resolveSOS = asyncHandler(async (req, res) => {
  ...
  const timeToResolution = (Date.now() - sos.createdAt) / 1000;
...
export const getPendingSOS = asyncHandler(async (req, res) => {
  try {
    ...
    throw new ApiError(500, error.message || "Failed to retrieve pending SOS alerts");
  }
});
...
export const getSOSById = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const currentUserId = req.user?._id?.toString();
    const isBroadcaster = sos.broadcaster?._id?.toString() === currentUserId;
    ...
    throw new ApiError(error.statusCode || 500, error.message || "Error fetching SOS details");
  }
});
...
export const respondToWelfareCheck = asyncHandler(async (req, res) => {
  const { sosId } = req.params;
  const { response } = req.body; // 'fine' or 'need_help'
