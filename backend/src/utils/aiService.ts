...
import type { CrisisType } from "../constant.js";
import type { ISOSGuidance } from "../models/sos.model.js";
...
const FALLBACK_GUIDANCE: Record<CrisisType, ISOSGuidance> = {
  medical: {
    steps: [
...
export async function generateCrisisGuidance(
  crisisType: CrisisType,
  address?: string
): Promise<ISOSGuidance> {
...
Mention specific Indian emergency numbers in steps if relevant (100, 101, 102, 108).`;
  ...
  try {
    ...
    const parsed = JSON.parse(cleaned) as Partial<ISOSGuidance>;
    if (Array.isArray(parsed.steps) && typeof parsed.emergencyScript === 'string') {
      return { steps: parsed.steps, emergencyScript: parsed.emergencyScript };
    }
  } catch {
...
export async function generateDebriefPrompt(
  crisisType: CrisisType,
...
export async function askCrisisAssistant(
  crisisType: string,
  question: string,
