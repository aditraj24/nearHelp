...
function Dashboard() {
  ...
  const handleAddGuardian = async () => {
    ...
    try {
      ...
      setPopup({ type: 'error', message: e.response?.data?.message || 'Failed to add guardian' });
    } finally {
  ...
  const handleWelfareResponse = async (sosId, response) => {
    try {
  ...
  useEffect(() => {
    ...
    socket.on('guardian_sos_alert', (alert) => {
      ...
      import('../services/offlineSOSQueue.js').then(({ syncOfflineQueue, clearSyncedQueue }) => {
        syncOfflineQueue().then((results) => {
          ...
          if (synced.length > 0) {
            ...
            if (synced[0]?.sosId) navigate(`/sos/${synced[0].sosId}`);
          }
        });
      });
    }
  ...
  const handleRespondNearest = (sosId) => {
    acceptSOS(sosId);
    navigate(`/sos/${sosId}`);
  };
              ...
              {!nearestSOS ? (
                ...
                <div className="relative isolate">
                  <div className="h-64 md:h-72 w-full z-0">
                    <MapContainer
                      center={[nearestSOS.location.coordinates[1], nearestSOS.location.coordinates[0]]}
                      zoom={14}
                    ...
                    >
                      ...
                      <Marker
                        position={[nearestSOS.location.coordinates[1], nearestSOS.location.coordinates[0]]}
                        icon={sosPulseIcon}
                      />
                    </MapContainer>
                  </div>
