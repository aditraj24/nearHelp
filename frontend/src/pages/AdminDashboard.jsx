...
import {
  Users, AlertTriangle, Activity, MapPin, Search, Filter, CheckCircle, XCircle,
  Menu, Bell, LogOut, LayoutDashboard, Shield, Clock, Flag, Map as MapIcon, Eye
} from 'lucide-react';
...
function AdminDashboard() {
  ...
  const fetchData = async () => {
    try {
      ...
      setSosList(sosRes.data.data.sosList || sosRes.data.data.sos || []);
  ...
  const fetchSOSList = async () => {
    try {
      const params = { limit: 20, page: sosPage };
      if (sosFilter) params.status = sosFilter;
      const res = await adminAPI.getAllSOS(params);
      setSosList(res.data.data.sosList || res.data.data.sos || []);
    } catch (error) {
                ...
                <>
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    ...
                    <StatCard
                      ...
                      value={users.filter(u => u.skills?.length > 0).length}
                      icon={Activity}
                      color="bg-purple-500"
                    />
                  </div>
                          ...
                          {filteredUsers.map(user => (
                            <tr key={user._id} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                              ...
                              <td className="px-6 py-4">
                                <span className={`px-2 py-1 rounded text-[10px] font-bold uppercase ${user.role === 'admin' ? 'bg-purple-100 text-purple-700' : 'bg-blue-50 text-blue-600'}`}>
                                  {user.role}
                                </span>
                              </td>
                              <td className="px-6 py-4">
                                <span className={`px-2 py-1 rounded text-[10px] font-bold uppercase ${user.isSuspended ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'}`}>
                              ...
                              <td className="px-6 py-4 text-right">
                                <button
                                  ...
                                  className={`text-xs font-medium px-3 py-1 rounded-lg border transition-colors ${user.isSuspended
